...
import { loadState, saveState } from './intake';
import {
  getDriveClient, readJsonFile, updateJsonFile,
  uploadTextFile, copyFileToDrive, getFolderWebLink,
} from './drive';
import { notifyApprovalComplete } from './slack';
import { EpisodeData, ApprovalStatus } from './types';
...
function authMiddleware(req: Request, res: Response, next: NextFunction): void {
  ...
  const cookie = (req as Request & { cookies: Record<string, string> }).cookies?.zoomies_session;
...
// ── Episode discovery from Drive ─────────────────────────────────────────────

interface EpisodeCard {
  folderId: string;
  shotListFileId: string;
  show: string;
  season: number;
  episode: number;
  title: string;
...
let episodeCache: EpisodeCard[] | null = null;
let episodeCacheAt = 0;
const CACHE_TTL_MS = 60_000;

async function discoverEpisodes(): Promise<EpisodeCard[]> {
  if (episodeCache && Date.now() - episodeCacheAt < CACHE_TTL_MS) return episodeCache;

  const rootId = process.env.ZOOMIES_DRIVE_ROOT_FOLDER_ID;
  if (!rootId) throw new Error('ZOOMIES_DRIVE_ROOT_FOLDER_ID not set');
  ...
  for (const file of res.data.files || []) {
    try {
      const ep = await readJsonFile<EpisodeData>(drive, file.id!);
  ...
  cards.sort((a, b) =>
    a.show.localeCompare(b.show) || a.season - b.season || a.episode - b.episode
  );
...
async function getSceneFolderMap(
...
async function getEpisodeFolders(episodeFolderId: string) {
  const drive = await getDriveClient();
  const res = await drive.files.list({
...
<body>
  ...
  // ── Directory (home) ────────────────────────────────────────────────────────
  app.get('/', (_req, res) => {
    const htmlPath = path.join(__dirname, '..', 'ui', 'directory.html');
    res.sendFile(path.resolve(htmlPath));
  });

  // ── Approval UI (per episode) ───────────────────────────────────────────────
  app.get('/episode/:folderId', (_req, res) => {
    const htmlPath = path.join(__dirname, '..', 'ui', 'approval.html');
    res.sendFile(path.resolve(htmlPath));
  });
  ...
  app.get('/api/episode/:folderId', async (req, res) => {
    try {
      ...
      const episode = await readJsonFile<EpisodeData>(drive, file.id!);
  ...
  app.post('/api/submit/:folderId', async (req, res) => {
    try {
      const { folderId } = req.params;
      const { shots, shotListFileId } = req.body as {
        shots: Array<{ id: string; status: ApprovalStatus; rejection_notes?: string }>;
        shotListFileId: string;
      };

      const drive = await getDriveClient();
      const episode = await readJsonFile<EpisodeData>(drive, shotListFileId);
      const folders = await getEpisodeFolders(folderId);
      ...
      episode.shots = episode.shots.map((shot) => {
        ...
        if (d.status === 'REJECTED') {
          rejectedCount++;
          if (d.rejection_notes) rejectionLines.push(`${shot.id}: ${d.rejection_notes}`);
        }
        return { ...shot, approval_status: d.status, rejection_notes: d.rejection_notes };
      });

      await updateJsonFile(drive, shotListFileId, episode);
      invalidateCache();
          ...
          if (shot.approval_status === 'APPROVED' && shot.prompt_drive_file_id) {
            try {
              await copyFileToDrive(
                drive, shot.prompt_drive_file_id,
                `${showCode}_S${episode.season}E${episode.episode}_${shot.scene_id}_${shot.id}_PROMPT.txt`,
                folders.approvedFolderId
              );
              moved++;
            } catch { /* non-fatal */ }
          }
      ...
      if (rejectionLines.length > 0 && folders.workingFolderId) {
        await uploadTextFile(drive, 'REJECTED_NOTES.txt', [
      ...
      try {
        const approvedLink = folders.approvedFolderId
          ? await getFolderWebLink(drive, folders.approvedFolderId) : '';
        await notifyApprovalComplete(episode, approvedCount, rejectedCount, approvedLink, rejectionLines.length > 0, folderId);
      } catch { /* non-fatal */ }
  ...
  app.get('/api/episode', async (_req, res) => {
    try {
      ...
      if (!state) { res.status(404).json({ error: 'No local state' }); return; }
      const drive = await getDriveClient();
      const episode = await readJsonFile<EpisodeData>(drive, state.shotListJsonFileId);
      res.json({
        episode,
  ...
  app.post('/api/submit', async (req, res) => {
    ...
    try {
      const drive = await getDriveClient();
      const episode = await readJsonFile<EpisodeData>(drive, shotListFileId);
      const approvalMap = new Map(shots.map((s: { id: string; status: ApprovalStatus; rejection_notes?: string }) => [s.id, s]));
      let approvedCount = 0, rejectedCount = 0;
      const rejectionLines: string[] = [];
      episode.shots = episode.shots.map((shot) => {
        const d = approvalMap.get(shot.id);
        if (!d) return shot;
        if (d.status === 'APPROVED') approvedCount++;
        if (d.status === 'REJECTED') { rejectedCount++; if (d.rejection_notes) rejectionLines.push(`${shot.id}: ${d.rejection_notes}`); }
        return { ...shot, approval_status: d.status, rejection_notes: d.rejection_notes };
      });
      await updateJsonFile(drive, shotListFileId, episode);
      invalidateCache();
      ...
      for (const shot of episode.shots) {
        if (shot.approval_status === 'APPROVED' && shot.prompt_drive_file_id && state.approvedFolderDriveId) {
          try { await copyFileToDrive(drive, shot.prompt_drive_file_id, `${showCode}_S${episode.season}E${episode.episode}_${shot.scene_id}_${shot.id}_PROMPT.txt`, state.approvedFolderDriveId); moved++; } catch { /* */ }
        }
      }
      if (rejectionLines.length > 0) await uploadTextFile(drive, 'REJECTED_NOTES.txt', [`REJECTED SHOTS — S${episode.season}E${episode.episode} ${episode.title}`, `Generated: ${new Date().toISOString()}`, '', ...rejectionLines].join('\n'), state.workingFolderDriveId);
      state.episode = episode; state.lastUpdated = new Date().toISOString(); saveState(state);
      const approvedLink = await getFolderWebLink(drive, state.approvedFolderDriveId);
      try { await notifyApprovalComplete(episode, approvedCount, rejectedCount, approvedLink, rejectionLines.length > 0, state.episodeFolderDriveId); } catch { /* */ }
  ...
  app.get('/api/audio/:fileId', async (req, res) => {
    try {
      ...
      const audioRes = await drive.files.get(
        { fileId, alt: 'media' }, { responseType: 'stream' }
      );
      res.setHeader('Content-Type', 'audio/wav');
      res.setHeader('Cache-Control', 'public, max-age=3600');
      (audioRes.data as NodeJS.ReadableStream).pipe(res);
  ...
  app.post('/api/rerun/:folderId', async (req, res) => {
    try {
      const { folderId } = req.params;
      const { shotUpdates, shotListFileId } = req.body as {
        shotUpdates: Array<{ id: string; new_angle?: string; rejection_notes?: string }>;
        shotListFileId: string;
      };
      ...
      const episode = await readJsonFile<EpisodeData>(drive, shotListFileId);
      const folders = await getEpisodeFolders(folderId);
      if (!folders.workingFolderId) throw new Error('Working folder not found');
      ...
      const shotsToRerun = episode.shots
        ...
        .map((s) => {
          ...
          return { ...s, angle: (upd.new_angle || s.angle) as EpisodeData['shots'][0]['angle'] };
        });
      ...
      const miniEpisode: EpisodeData = { ...episode, shots: shotsToRerun };

      // Regenerate prompts
      const { buildAndUploadPrompts } = await import('./prompts');
      ...
      // Regenerate images
      const { generateAndUploadImages } = await import('./images');
      const { results: imageResults } = await generateAndUploadImages(
        miniEpisode, sceneFolderMap, folders.workingFolderId
      );
      ...
      episode.shots = episode.shots.map((s) => {
        ...
        return {
          ...s,
          angle: (upd.new_angle || s.angle) as EpisodeData['shots'][0]['angle'],
          prompt_drive_file_id: newPromptIds[s.id] ?? s.prompt_drive_file_id,
